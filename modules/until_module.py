...
class PreTrainedModel(nn.Module):
    ...
    @classmethod
    def init_preweight(cls, model, state_dict, prefix=None, task_config=None, pre_trained_model='pre_trained model'):
        old_keys = []
        new_keys = []
        ...
        #<== test
        #before_load = copy.deepcopy(model.state_dict())
        # # pdb.set_trace()
        
        load(model, prefix='')
        # for key in model.state_dict().keys():
        #     if  key not in state_dict.keys():
        #         print(key, ' not in state dict.')
        #         continue
           
        #     if torch.equal(model.state_dict()[key] , before_load[key]):
        #         print(key, ' not load from saved_weight.')
        #     elif torch.equal(model.state_dict()[key], state_dict[key]):
        #         print(key, ' load from saved_weight.')
        #     else:
        #         pdb.set_trace()
        # ==> test#
        if prefix is None and (task_config is None or task_config.local_rank == 0):
            ...
            if len(missing_keys) > 0:
                logger.info("Weights of {} not initialized from {}: {}"
                            .format(model.__class__.__name__,pre_trained_model, "\n   " + "\n   ".join(missing_keys)))
            if len(unexpected_keys) > 0:
                logger.info("Weights from {} not used in {}: {}"
                            .format(model.__class__.__name__, pre_trained_model, "\n   " + "\n   ".join(unexpected_keys)))
            if len(error_msgs) > 0:
                logger.error("Weights from {} cause errors in {}: {}"
                             .format(model.__class__.__name__,pre_trained_model,"\n   " + "\n   ".join(error_msgs)))
...
class CrossEn(nn.Module):
    ...
    def forward(self, sim_matrix, mask=None):
        ...
        if mask is None:
            ...
            logpt=logpt[mask==True]
            nce_loss = -logpt
            sim_loss = nce_loss.mean()
        
        return sim_loss

class HardCrossEn(nn.Module):
    def __init__(self,):
        super(HardCrossEn, self).__init__()
...
class MILNCELoss(nn.Module):
    def __init__(self, batch_size=1, n_pair=1,):
        ''' MIL NCE Loss for 2-momdal joint loss, n_pair means the number of positive samples'''
    ...
    def forward(self, sim_matrix, gt=None):
        '''
        text 与video 的MIL-NCE loss， text1， video 012 为正样例，其他video，所有text为负样例
        
        '''
        
        mm_mask = np.eye(self.batch_size)# [batchsize, batchsize]
        
        mm_mask = np.kron(mm_mask, np.ones((self.n_pair, self.n_pair))) # [batchsize*3, batchsize*3]
...
class MaxMarginRankingLoss(nn.Module):
    ...
    def forward(self, x, gt=None):
        ...
        d = torch.diag(x)
        if gt == None:
            gt = torch.eye(x.shape[0],device=x.device)
            
        mask = torch.ones_like(x,dtype=x.dtype, device=x.device)-gt
        #Relu(margin + sim<i,j> - sim<i,i> )+ Relu(margin + sim<i,j> - sim<j,j>)
        max_margin = F.relu(self.margin + x -  d.view(-1, 1)) + \
                     F.relu(self.margin + x -  d.view(1, -1))
        max_margin = max_margin * mask
...
class Loss_recoder():
    ...
    def update(self,key,value):
        if key in self.loss.keys():
            ...
            self.loss[key] = value.item()
        if 'tol' in self.loss.keys():
            self.loss['tol'] += value.item()
        else:
            self.loss['tol'] = value.item()
    ...
    def tol(self):
        return self.loss['tol']
        
    def return_str(self):
        str_list = []
        for key, val in self.loss.items():
            if key != 'tol':
                str_list.append("{}:{:.3f}".format(key,val)) 
            

        loss_str=" ".join(str_list)

        return loss_str

    def mean(self, div):
        assert div !=0, 'The divisor cannot be 0'
