...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        if self.with_control_token > -1 :
            self.control_embedding = nn.Parameter(scale * torch.randn(2, width))
            
            self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // patch_size) ** 2 + 2, width))
        else:
    ...
    def forward(self, x: torch.Tensor, token_type=None, video_frame=-1):
...
class CLIP4Audio(nn.Module):
    ...
    def encode_audio_transformer(self, audio, token_type=None,return_hidden=False, audio_frame=-1):
        # transfer audio to 
        # image = torch.zeros_like(image, dtype=image.dtype, device=image.device)
     
        hidden = self.visual(audio.type(self.dtype), token_type=token_type,video_frame = audio_frame)
        ...
        if return_hidden:
            return x, bg_token, hidden

        return x, bg_token

    def forward(self, audio, token_type=None,audio_frame=-1):
        audio_hidden, bg_token = self.encode_audio_transformer(audio,token_type=token_type, audio_frame = audio_frame)
        audio_hidden = audio_hidden.float()     
        bg_token = bg_token.float()   
        return audio_hidden, bg_token
